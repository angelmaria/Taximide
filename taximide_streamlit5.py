# con sidebar en la derecha para el registro de activadad
...
def cambiar_contraseña():
    nueva_contraseña = st.text_input("Ingresa la nueva contraseña:", type="password", max_chars=12, key="new_password_input")
    confirmar_contraseña = st.text_input("Confirma la nueva contraseña:", type="password", max_chars=12, key="confirm_password_input")
...
def finalizar_programa():
    ...
    c.execute('''INSERT INTO registros (timestamp, tiempo_total, total_a_cobrar, tarifa) VALUES (?, ?, ?, ?)''',
              (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), st.session_state.tiempo_total, st.session_state.total_a_cobrar, tarifa_actual))
    conn.commit()
...
def pagina_inicio():
    ...
    st.markdown("""<style>.big-font {font-size:30px !important; font-weight: bold; color: #1E90FF;}</style>""", unsafe_allow_html=True)
    st.markdown('<p class="big-font">Bienvenido al Taxímetro Digital</p>', unsafe_allow_html=True)

    contrasena = st.text_input("Ingresa la contraseña:", type="password", max_chars=12, key="login_password_input")
...
def pagina_principal():
    ...
    with col1:
        ...
        st.markdown(f"""<div style='border: 2px solid #1E90FF; border-radius: 10px; padding: 10px; margin-bottom: 20px;'>
                         <h3 style='color: #1E90FF;'>Tarifas actuales:</h3>
                         <p>Parado: {st.session_state.tarifa_parada:.2f} €/segundo</p>
                         <p>Movimiento: {st.session_state.tarifa_movimiento:.2f} €/segundo</div>""", unsafe_allow_html=True)
    ...
    with col2:
        ...
        with open('taximetro.log', 'r') as file:
            log_records = file.read()
            
        st.title("Registro de Actividad")
        st.text_area("Registros", log_records, height=600)
