# streamlit con los contadores en cajas (boxes)
...
def reiniciar_carrera():
    ...
    st.session_state.autenticado = False   # Reiniciar el estado de autenticación
    st.success("Carrera reiniciada. Por favor, ingresa la contraseña para comenzar una nueva.")
...
def pagina_principal():
    ...
    # Estilo para contadores como "flip count up clock"
    st.markdown("""
    <style>
    .flip-clock {
    ...
    # Mostrar contadores con estilo flip clock
    st.markdown(f"<div class='flip-clock'><span class='flip-clock-label'>Tiempo total:</span> {int(st.session_state.tiempo_total)} segundos</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='flip-clock'><span class='flip-clock-label'>Total a cobrar:</span> {st.session_state.total_a_cobrar:.2f} €</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='flip-clock'><span class='flip-clock-label'>Tarifa:</span> {tarifa_actual:.2f} €/seg</div>", unsafe_allow_html=True)
